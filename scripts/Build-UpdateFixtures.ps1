...
# No test-only updater bypass or modified executable bytes are introduced.
param()
...
foreach ($fixture in $fixtures) {
    ...
    & dotnet publish $source -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -p:IncludeNativeLibrariesForSelfExtract=true -p:DebugType=none ('-p:Version=' + $fixture.Version) ('-p:InformationalVersion=' + $fixture.InformationalVersion) -o $destination
    ...
    $manifest += [pscustomobject]@{ name = $fixture.Name; version = $fixture.Version; relativePath = $fixture.Name + '/RemoteDebugger.exe'; sha256 = $signed.Sha256; publisherSha256 = $signed.PublisherSha256 }
}
