...
public static class WorkspacePresentation
{
    ...
    {
        1 => (UiText.RemoteScreen, screen),
        2 => (resources, measurements),
        3 => (files, string.IsNullOrWhiteSpace(directory) ? UiText.Workspace : directory),
        4 => (diagnostics, UiText.ActionPrefix + operation),
        _ => (connection, string.IsNullOrWhiteSpace(host) ? UiText.EnterRemoteAddress : UiText.PcPrefix + host)
    };

    public static string Identity(bool session, bool healthy, string host, string fingerprint) => !session
        ? UiText.NoAuthenticatedConnection
        : $"{(healthy ? UiText.VerifiedIdentity : UiText.LastVerifiedIdentity)} · {host} · {fingerprint[..Math.Min(16, fingerprint.Length)]}…";

    public static string ArgumentsFor(string template, int pid)
    {
        var value = JsonNode.Parse(template) as JsonObject ?? throw new JsonException(UiText.ArgumentsMustBeObject);
