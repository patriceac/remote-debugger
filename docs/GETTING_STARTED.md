...
2. On first use, **Enable support** performs the one-time Windows administrator setup. It installs the protected support application and a local broker, enrolls the publisher, and relaunches the managed copy. Thereafter Private-network authorization, LAN fallback and administrator maintenance are automatic. The application does not change a Public network to Private.
