...
Files: Remote folder label and full path input + Go/Parent/Refresh; below full table Name / Type / Size / Modified with typed sorting and folders grouped where appropriate. Upload file and Upload folder target the open remote folder; Download saves the selected file to a local destination chosen in the native dialog. Separate the directory from the selected file, with no second upload-destination editor. Initial remote workspace listing loads upon connection. Inaccessible/empty/loading states are distinct. Reserve a compact transfer area below the table with progress bar, percentage, bytes, speed, ETA and Cancel. Folder progress covers all selected bytes; preparation and verification have their own states. Stable IDs are remoteFolderLabel, openRemoteFolder, fileTransferProgress, fileTransferStatus, fileTransferDetails and cancelTransfer; destination/destinationLabel are retired.
