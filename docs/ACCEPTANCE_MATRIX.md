...
| `agent.sleep_idle` | An idle listener may sleep while waiting for a controller | Read-only `powercfg /requests` evidence shows no RemoteDebugger request before pairing, and the visible state is active | Runtime |
| `agent.sleep_request` | Sleep is held during authenticated support | Product-owned `powercfg /requests` evidence and the visible state show the system request after the agent is paired and synchronized | Runtime |
...
| `agent.maintenance_after_pairing` | Prepared admin maintenance is available after pairing | Agent state, service/broker status, and paired maintenance command show authorized maintenance with no repeated UAC prompt | Provisioned |
| `input.prepared_before_pairing`, `input.first_input`, `input.reconnect`, `input.new_session` | Input is prepared before pairing and reuses the same helper across sessions | Signed Release input review checks helper identity and input latency below three seconds | Provisioned |
| `input.unauthorized_denied`, `input.ended_grant_revoked`, `input.disconnect_reuses_helper`, `input.disable_teardown`, `input.quit_teardown` | Warm input retains authorization and teardown boundaries | Empty and revoked grants fail; disconnect releases held keys; disable and quit remove the helper | Provisioned |
