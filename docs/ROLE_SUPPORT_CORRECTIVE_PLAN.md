...
The existing remote-update proof and strict-newer policy reject agent callers and older/equal candidates. Full authorized forward update, reconnect and identical-byte no-op worked in the disposable fixture. However, a controller with its receiving switch Off accepted a full newer update and subsequent support from another controller; its saved Off value survived.

Normal launch of the exact installer requested elevation, received consent, installed the application, then exited 9 because its support service could not be installed/refreshed. The helper was absent. An independent repeat with a successful harness contract confirms the evaluated product failure. Source inspection identifies a strong explanation: the original user's provision-request writer is directed into Inno's protected elevated temporary directory, where its medium token cannot obtain write access through the administrator group. The underlying file-access exception was not captured. Explicitly pre-elevated installation and agent reinstall succeeded, which is consistent with this handoff problem. A separate same-source fixture provision failed SCM 1053 despite elevation; keep that service-startup observation distinct.
...
| P1 | Preserve automatic installer elevation and correct the original-user provisioning handoff. Use a narrowly scoped request exchange writable by the intended initiating user, retaining requester SID/PID/start-time/executable-hash validation. Surface request-writer failure promptly. Keep Inno's protected temporary-directory permissions intact. | Inno token handoff, original-user setup operations, protected helper/service provisioning. | I01-I05: normal launch with consent succeeds; standard-user credentials act on the correct profile; UAC decline leaves no false-ready installation; explicit-elevation comparison matches. Capture stage-specific failures and prove the helper is ready. |
...
| Original-user request targets protected setup temporary directory | `installer/RemoteDebugger.iss` (`RequestPath`, `ExecAsOriginalUser`); `src/RemoteDebugger/SupportInstaller.cs` (`WriteInstallerProvisionRequest`) | Confirm the captured exit-9 helper failure, then correct only the request exchange and its error reporting while preserving identity validation. Do not weaken Inno's directory protection. Keep the separate elevated SCM 1053 service-start failure visible. |
...
Do not treat a test infrastructure limitation as a product failure. Patrice separately authorized the Hyper-V Harness project to implement, qualify and deploy its missing capabilities with Astra Max. Its V2 release now supports installer self-elevation, standard-user credentials and UAC decline. Exact setup decline passed; the first standard-user consumer run exposed a hardcoded secure-helper deadline and missing intermediate receipts. A narrow follow-up is being qualified before that test resumes. VM isolation, exact artifact identity, cleanup, and permission boundaries must remain intact. This authorization does not permit Remote Debugger product fixes.
