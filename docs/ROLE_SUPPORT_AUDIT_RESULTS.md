...
Follow-up in progress: the Hyper-V Harness project completed its Astra Max extension, eleven live acceptance paths and recovery qualification. Deployment `deploy-4e17f36b76bb989d` (source `d585b981ed68c98c801b9a0815465488b735a422`) reached Ready on 2026-09-25 at 07:24:44 UTC. Original-setup UAC decline (I04) passed. Normal-launch I01 now has a clean evaluated failure: its own UAC succeeds, then helper installation fails with setup exit 9. I03 stopped at a harness secure-prompt deadline. The local relay stopped at a diagnostic runtime mismatch before policy checks. Both follow-ups are held for the harness's narrow next release. Product code and original installer bytes are unchanged.
...
Of 37 tracked scenarios, **18 fail the agreed requirements, 14 pass within their stated scope, four are partial, and one is blocked by the harness**. These are scenario outcomes, not counts of independent bugs. B03's failure is established in unit scope; its packaged relay coverage remains open. Product code and the original release/installer bytes remain unchanged.
...
| C04 | PASS, tested positive control | Controller to agent screen capture/files/processes/commands/admin operations work; supplementary actual text input also succeeds. A separate source-receiving-OFF variant is prepared but unrun. |
...
| I03 | BLOCKED, harness secure-prompt deadline | Real standard-user preparation and native Inno UAC attribution succeeded. The secure helper hit its hardcoded 25-second wait; decision/elevation/After receipts are missing, so the actual input outcome is unknown. Harness project is correcting and qualifying this path with Astra Max. |
...
- **Standard-user credentials (I03):** needs a qualified harness deadline/evidence correction before retry. Normal-launch I01 now confirms automatic elevation followed by helper failure; exact setup UAC decline (I04) is verified. The protected temporary-directory handoff is a strong source-derived explanation for I01, not a captured file-access exception or proof of the historical family-PC cause.
...
- **Outgoing support with the source's receiving setting OFF:** the current lifecycle tests verify the setting and available outgoing UI, but do not establish an actual outgoing session in this exact source state. A separate C04 positive-control variant is prepared for the next VM window. Existing S01 already proves saved-token reuse after the *target* switches OFF through a new CLI process/transport; long-running GUI/heartbeat/input termination and automatic reconnect variants remain open.
- **Transport and entry-point scope:** relay directory/channel behavior was exercised in the local worker simulator; the complete packaged local-relay journey is still unverified. The first attempt stopped at certificate-trust consent; the second could not read the elevated setup's receipt; the third passed that handoff but its Miniflare runtime rejected the diagnostic's old configuration API. None reached a packaged relay policy case. The full isolated LAN discovery matrix, original packaged MCP and saved-grant reuse after authority removal/process restart are verified. The LAN capture retains a Windows Firewall prompt and `firewallReady:false`, so it does not qualify production firewall setup. No deployed Cloudflare journey or every restart/resume variant has been claimed.
...
- Both requests completed process/VM/payload/network cleanup. The harness project is implementing and qualifying a bounded deadline plus non-secret checkpoint correction; I03 stays held until its next deployed-ready handoff. No Remote Debugger source correction is authorized.
...
- These attempts use the same-source disposable-authority .21 fixture and a local Miniflare/workerd relay with no Internet route. None is evidence of deployed Cloudflare behavior. Diagnostic source is being adapted using the exact package's exported `convertV4MiniflareOptions` compatibility converter; the product relay source is unchanged. Further submissions are held around harness maintenance.
