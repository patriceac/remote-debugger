...
The relay limits an invitation to twelve concurrent request/stream channels, binary messages to 64 KiB and each channel to 16 MiB per second. Pending channels expire. WebSocket hibernation keeps idle connections inexpensive; payloads are never written to Durable Object storage. Discovery results are privately cached for up to five seconds and ended registrations are removed. Free-tier limits apply to the whole Cloudflare account. Requests can fail when that day's free quota is exhausted. Registered device count alone does not guarantee free operation: measure actual session duration and stream traffic.

Heartbeats reuse an authenticated channel while retaining the five-second interval and fifteen-second loss detection. Established TLS writes are combined into bounded WebSocket messages and flushed at protocol boundaries. Updates negotiate binary chunks, retaining base64 compatibility with older agents. Sustained relay failures use randomized exponential retry delays capped at five minutes. Normal minimization and hiding to the tray pause screen traffic without ending support. The enabled-by-default **Economy mode** option limits relay viewing to 1280 pixels wide at up to 3 FPS; direct LAN/WAN viewing retains its normal settings. Turn the option off for full relay viewing quality.
